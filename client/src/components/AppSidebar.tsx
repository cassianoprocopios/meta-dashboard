  ...
  function SidebarContent({ isMobile = false }: { isMobile?: boolean }) {
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-between px-3.5 py-4 border-b border-white/[0.08]">
          {!isCollapsed && (
            <div className="flex items-center gap-2 overflow-hidden">
              <div className="w-8 h-8 rounded-[10px] bg-white/10 ring-1 ring-white/10 flex items-center justify-center flex-shrink-0">
                <TrendingUp className="w-3.5 h-3.5 text-emerald-300" />
              </div>
              <div className="min-w-0">
                <p className="text-xs font-semibold text-white leading-none truncate">Meta Dashboard</p>
                <p className="text-[9px] uppercase tracking-[0.11em] text-white/35 leading-none mt-1 truncate">Performance OS</p>
              </div>
            </div>
          )}
          {isCollapsed && (
            <div className="w-8 h-8 rounded-[10px] bg-white/10 ring-1 ring-white/10 flex items-center justify-center mx-auto">
              <TrendingUp className="w-3.5 h-3.5 text-emerald-300" />
            </div>
          )}
          ...
          {!isMobile && isCollapsed && (
            <button
              ...
              className="absolute -right-3 top-[52px] w-6 h-6 rounded-full bg-[#12233f] border border-white/15 flex items-center justify-center text-white/60 hover:text-white hover:border-white/30 transition-all shadow-lg z-10"
              title="Expandir menu"
            >
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden py-3 space-y-1 px-2 scrollbar-none">
          {/* Módulos principais */}
          {!isCollapsed && (
            <p className="text-[9px] font-semibold text-white/30 tracking-[0.15em] uppercase px-2 pb-1">
              Módulos
            </p>
          )}
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${
                    isActive
                      ? "bg-white/10 text-white shadow-[inset_3px_0_0_#34d399]"
              ...
              >
                ...
                {isActive && !isCollapsed && (
                  <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-emerald-300" />
                )}
                <span className={`flex-shrink-0 transition-transform duration-150 ${isActive ? "text-emerald-300" : "text-white/45"}`}>
                  {item.icon}
                </span>
                {!isCollapsed && (
                  <div className="flex-1 text-left min-w-0">
                    ...
                    <p className={`text-[10px] truncate leading-none mt-0.5 transition-colors duration-150 ${isActive ? "text-white/45" : "text-white/25"}`}>
                      {item.description}
                    </p>
                  </div>
                )}
              ...
              <Link key={link.href} href={link.href}>
                <button
                  ...
                  className={`
                    ...
                    ${
                      isActive
                        ? "bg-blue-500/15 text-white border border-blue-300/15"
                        : "text-white/50 hover:text-white/90 hover:bg-white/[0.05] border border-transparent"
                    }
                  `}
                >
                  <span className={`flex-shrink-0 ${isActive ? "text-blue-300" : "text-white/45"}`}>
                    {link.icon}
                  </span>
                  {!isCollapsed && (
                    <div className="flex-1 text-left min-w-0">
        ...
        <div className="border-t border-white/[0.06] p-2">
          {!isCollapsed ? (
            <div className="flex items-center gap-2 px-2 py-2">
              <div className="w-8 h-8 rounded-[10px] bg-emerald-400/15 ring-1 ring-emerald-300/10 flex items-center justify-center flex-shrink-0">
  ...
  return (
    <>
      ...
      <button
        ...
        className="md:hidden fixed top-3 left-3 z-50 w-9 h-9 rounded-[10px] bg-white border border-slate-200 flex items-center justify-center text-[#12233f] hover:bg-slate-50 shadow-sm"
      ...
      {mobileOpen && (
        <div
          className="md:hidden fixed inset-0 z-40 bg-slate-950/45 backdrop-blur-sm"
          onClick={() => setMobileOpen(false)}
        />
      )}
      ...
      <div
        className={`
          ...
          bg-[#0b1830] border-r border-white/[0.08] flex flex-col
      ...
      <aside
        className={`
          ...
          bg-[#0b1830] border-r border-white/[0.08] shadow-[8px_0_30px_rgba(15,23,42,0.06)]
