...
interface FaturamentoRow {
  ...
  data: unknown;
  servicos: unknown;
  vendaProdutos: unknown;
  novasAssinaturas: unknown;
  recorrencia: unknown;
  observacao: unknown;
}
...
export default function FaturamentoForm({ mes, ano, initialData, existingData, onClose, onSaved }: Props) {
  ...
  const [activeEmpresa, setActiveEmpresa] = useState<Empresa>(initialData?.empresa || "MORUMBI");

  const salvarMutation = trpc.faturamento.salvar.useMutation();

  // Preencher com dados existentes quando a data muda
  useEffect(() => {
    ...
    for (const emp of EMPRESAS) {
      ...
      if (existing) {
        newValues[emp] = {
          servicos: String(parseFloat(String(existing.servicos || 0)) || ""),
          vendaProdutos: String(parseFloat(String(existing.vendaProdutos || 0)) || ""),
          novasAssinaturas: String(parseFloat(String(existing.novasAssinaturas || 0)) || ""),
  ...
  const handleChange = (emp: Empresa, field: keyof EmpresaValues, val: string) => {
    setValues((prev) => ({
      ...prev,
  ...
  const totalEmpresa = (emp: Empresa) => {
    const v = values[emp];
    return parseVal(v.servicos) + parseVal(v.vendaProdutos) + parseVal(v.novasAssinaturas) + parseVal(v.recorrencia);
  };
    ...
    for (const emp of EMPRESAS) {
      ...
      try {
        await salvarMutation.mutateAsync({
          empresa: emp,
          data: dataStr,
          servicos: parseVal(v.servicos),
          vendaProdutos: parseVal(v.vendaProdutos),
          novasAssinaturas: parseVal(v.novasAssinaturas),
          recorrencia: parseVal(v.recorrencia),
          observacao: v.observacao || undefined,
        });
        savedCount++;
      } catch (err) {
        toast.error(`Erro ao salvar ${emp}`);
        return;
      }
    }
      ...
      <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto border-0 shadow-2xl rounded-2xl bg-white">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-slate-50 rounded-xl p-4 space-y-4">
            ...
            {(["servicos", "vendaProdutos", "novasAssinaturas", "recorrencia"] as const).map((field) => {
              const labels: Record<string, string> = {
                servicos: "Serviços",
                vendaProdutos: "Venda de Produtos",
                novasAssinaturas: "Novas Assinaturas",
                ...
                <div key={field}>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      min="0"
                      placeholder="0,00"
                      value={values[activeEmpresa][field]}
                      onChange={(e) => handleChange(activeEmpresa, field, e.target.value)}
                      className="w-full pl-9 pr-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white"
                    />
                  </div>
                </div>
              );
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white resize-none"
              />
            </div>
          </div>

          {/* Resumo do dia */}
          {EMPRESAS.some((e) => totalEmpresa(e) > 0) && (
            <div className="bg-blue-50 rounded-xl p-4">
              <h4 className="text-xs font-semibold text-blue-700 uppercase tracking-wide mb-2">Resumo do Dia</h4>
              <div className="space-y-1">
                {EMPRESAS.filter((e) => totalEmpresa(e) > 0).map((emp) => (
                ...
                <div className="border-t border-blue-200 pt-1 mt-1 flex justify-between text-sm font-bold">
                  ...
                  <span className="text-blue-800">{fmt(EMPRESAS.reduce((s, e) => s + totalEmpresa(e), 0))}</span>
                </div>
              </div>
            </div>
          )}
