...
import { Shield, User, Building2, ChevronDown, Save, ArrowLeft } from "lucide-react";
import { toast } from "sonner";
import { Link } from "wouter";

type Empresa = "MORUMBI" | "MASCOTE" | "SERAPHINE" | null;
...
export default function AdminUsers() {
  ...
  const [editando, setEditando] = useState<Record<number, { perfil: Perfil; empresa: Empresa }>>({});

  if (user?.role !== "admin") {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 flex items-center justify-center p-4">
        <Card className="p-8 border-0 shadow-sm rounded-2xl text-center max-w-sm">
          <Shield className="w-12 h-12 text-red-400 mx-auto mb-4" />
          <h2 className="text-lg font-bold text-slate-900 mb-2">Acesso Restrito</h2>
          <p className="text-slate-500 text-sm mb-4">Esta página é exclusiva para administradores do sistema.</p>
          <Link href="/">
  ...
  const getEdit = (userId: number, currentPerfil: Perfil, currentEmpresa: Empresa) => {
  ...
  const setEdit = (userId: number, field: "perfil" | "empresa", value: string | null) => {
    const current = editando[userId] ?? { perfil: "operador" as Perfil, empresa: null as Empresa };
    setEditando((prev) => ({
      ...prev,
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-lg font-bold text-slate-900">Gestão de Usuários</h1>
                <p className="text-xs text-slate-500">Perfis e acessos por empresa</p>
              </div>
            </div>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-4">
        ...
        <Card className="p-4 border-0 shadow-sm rounded-2xl bg-blue-50">
          <div className="flex flex-wrap gap-4 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-700"><strong>Sem empresa</strong> — acesso a todas as unidades (admin)</span>
            </div>
          </div>
        </Card>
        ...
        {usuarios.length === 0 ? (
          <Card className="p-12 border-0 shadow-sm rounded-2xl bg-white text-center">
            ...
            <p className="text-slate-400 text-sm mt-1">Os usuários aparecem aqui após o primeiro login.</p>
          </Card>
        ) : (
          usuarios.map((u) => {
            const currentPerfil = (u.perfil ?? "operador") as Perfil;
            const currentEmpresa = (u.empresaVinculada ?? null) as Empresa;
                      ...
                      <div className="flex items-center gap-1.5 py-2">
                        {currentEmpresa ? (
                          <div className="flex items-center gap-1.5">
                            <Building2 className="w-3.5 h-3.5" style={{ color: EMPRESA_COLORS[currentEmpresa] }} />
                            <span className="text-sm font-medium" style={{ color: EMPRESA_COLORS[currentEmpresa] }}>
                              {currentEmpresa.charAt(0) + currentEmpresa.slice(1).toLowerCase()}
                            </span>
                          </div>
                        ) : (
