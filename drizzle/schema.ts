...
// ─── CONFIGURAÇÃO AVEC (Seraphine) ────────────────────────────────────────────
// Cada registro representa uma empresa integrada ao Avec
export const avecConfig = mysqlTable("avecConfig", {
  id: int("id").autoincrement().primaryKey(),
  tenantId: int("tenantId").notNull(),
  empresaSlug: varchar("empresaSlug", { length: 64 }).notNull(),
  /** Email de login no Avec */
  avecEmail: varchar("avecEmail", { length: 320 }).notNull(),
  /** Senha de login no Avec */
  avecSenha: varchar("avecSenha", { length: 256 }).notNull(),
  /** ID do salão no Avec (ex: 95687 para Seraphine) */
  avecSalaoId: varchar("avecSalaoId", { length: 32 }).notNull(),
  /** Nome do salão no Avec para exibição */
  avecSalaoNome: varchar("avecSalaoNome", { length: 128 }),
  /** Data/hora da última sincronização bem-sucedida */
  ultimaSincronizacao: timestamp("ultimaSincronizacao"),
  /** Status da última sincronização */
  statusUltimaSinc: varchar("statusUltimaSinc", { length: 32 }),
...
export type AvecSyncLog = typeof avecSyncLog.$inferSelect;
export type InsertAvecSyncLog = typeof avecSyncLog.$inferInsert;

// ─── MAPEAMENTO DE CATEGORIAS AVEC ───────────────────────────────────────────
// Mapeia categorias do Avec (Serviços, Produtos, Caixinha, Pacotes) para categorias do Meta Dashboard
export const avecMapeamento = mysqlTable("avecMapeamento", {
  id: int("id").autoincrement().primaryKey(),
  tenantId: int("tenantId").notNull(),
  empresaSlug: varchar("empresaSlug", { length: 64 }).notNull(),
  /** Categoria do Avec: 'Serviços' | 'Produtos' | 'Caixinha' | 'Pacotes' */
  avecCategoria: varchar("avecCategoria", { length: 64 }).notNull(),
  /** Coluna de destino no faturamento: 'cat1' | 'cat2' | 'cat3' | 'cat4' | 'cat5' | 'ignorar' */
  metaCategoria: varchar("metaCategoria", { length: 16 }).notNull().default("cat1"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
